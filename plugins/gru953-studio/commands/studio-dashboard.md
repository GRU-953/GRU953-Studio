...
2. Run `node "${CLAUDE_PLUGIN_ROOT}/hooks/dashboard.mjs" .` — it reads
   `Dev-Memory/PROGRESS.md` and writes a self-contained
   `Dev-Memory/dashboard.html` (all styling inline, no internet needed, nothing
   changed). Never write the HTML by hand; the generator is what guarantees the
   page is safe and self-contained.
3. Tell the user in one or two sentences that the dashboard is ready and where
   ...
   browser to see every task and its status at a glance. It is private and
   never published.
