...
2. Via `memory-keeper`, set the active task back to `todo` (never leave a
   half-finished task marked `done`), checkpoint the memory files, refresh the
   `▶ RESUME HERE` pointer and `Dev-Memory/STATUS-BOARD.md`, and append a
   one-line note to `SESSION-LOG.md`. Never touch Publish or any push.
