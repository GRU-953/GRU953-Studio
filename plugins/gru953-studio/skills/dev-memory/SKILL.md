...
**Which files are DATA and which are VIEWS (v7.0.0).** Three files are authoritative
and machine-written — `run-brief.json`, `tasks.json`, `dod.json` — and three familiar
markdown files are now RENDERED from them: `OBJECTIVE.md`, `PROGRESS.md` and
...
mistake of using a human presentation format as a data structure and then discovering
people write it eleven ways. A rendered file cannot be torn, because nothing parses it
back.
...
| `PROGRESS.md`             | **GENERATED from `tasks.json` by `hooks/task-ledger.mjs`. Do not edit — hand edits are overwritten.** The readable task table, with the `▶ RESUME HERE` marker on the next runnable task. It stays in the shape `hooks/verify-progress.mjs` reads, so a `done` row still carries its `verified:` evidence cell.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `QUALITY-GATE.md`         | **GENERATED from `evidence/*.json` by `hooks/dod.mjs`. Do not edit.** The readable Definition-of-Done table, written from measured exit codes rather than composed by the agents being graded. A standard the work can edit is not a standard.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
3. This **local** commit happens unconditionally, on every write, on every
   ...
   that reading directly contradicted "there is no _automatic_ backup"
...
persistence" above), the rehearsal additionally proves the _branch-persisted_
...
published product, and there is no _automatic_ backup. Two rules keep it
private:
...
- **Intended for a private branch — and read the correction below before
  ...
  **Corrected 2026-08-18 (X225). This paragraph used to end: "that `gate.mjs`
  accepts for an ordinary (private) push only — checked _after_ the go-public
  ...
  actually does is narrow: it stops `scan.mjs` objecting _purely because_ a
...
- **Each push updates that one branch, not a growing pile of snapshots.**
  Because `memory/cloud-persist` always holds the _whole current_ Dev-Memory
...
The schema above is identical across every session and every machine — a new
session picking up an existing project reads the exact same file names, columns,
and `▶ RESUME HERE` convention a prior session left behind, so nothing is ever
lost between sessions. (It said "Claude Code and Google Antigravity" until
2026-08-27; Antigravity support was removed in 7.0.0 and the claim outlived it.)
