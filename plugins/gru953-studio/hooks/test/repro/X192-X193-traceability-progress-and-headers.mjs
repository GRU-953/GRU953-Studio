...
function verdict(mutate) {
  ...
  try {
    ...
    const v = refuseCrash(readGate(NODE, join(HOOKS, 'traceability-check.mjs'), [dir]), 'X192-X193', die);
...
if (!reqHeader || !firstTaskId) die(`cannot read the golden fixture's own shape (header=${reqHeader}, id=${firstTaskId}) — the fixture changed and this reproduction must be re-read before it is trusted.`);

const sepFor = (header) => `|${header.split('|').filter((s) => s.trim()).map(() => ' :-- ').join('|')}|`;
const appendTable = (file, header, row) => (dm) =>
  appendFileSync(join(dm, file), `\n## A later phase\n\n${header}\n${sepFor(header)}\n${row}\n`);

// ---- A: baseline ---------------------------------------------------------------
const A = verdict(null);
if (A.status !== 'clean') die(`control A failed: the golden fixture must be clean at baseline, got ${A.status}: ${A.problems[0] || ''}`);
console.log('  A  the golden fixture, untouched ............... clean   (control)');

// ---- B: the branch that already works -------------------------------------------
const B = verdict(appendTable('REQUIREMENTS.md', '| Requirement | ID | Tasks | Verification |', '| a later need | R90 | T1 | T1 |'));
if (!B.problems.some((p) => String(p).includes(MISMATCH_SAYS))) {
  die(`control B failed: a mismatched later table in REQUIREMENTS.md must be reported — that branch was fixed in round 1, so if it is quiet the shared reader has broken: ${B.problems[0] || ''}`);
}
console.log('  B  a mismatched later table in REQUIREMENTS.md . BLOCKED (control)');

// ---- C: X192 --------------------------------------------------------------------
const C = verdict(appendTable('PROGRESS.md', '| Task | ID | Status | Evidence |', `| a later task | ${firstTaskId} | done | proof |`));
const cCaught = C.problems.some((p) => String(p).includes(MISMATCH_SAYS));
console.log(`  C  the identical defect in PROGRESS.md ......... ${cCaught ? 'BLOCKED' : 'clean  '}${cCaught ? '' : '  <- X192'}`);
...
const D = verdict((dm) => appendFileSync(join(dm, 'PROGRESS.md'), `| ${firstTaskId} | a short row |\n`));
...
if (flagged(E)) die(`control E failed: a later section repeating the header VERBATIM must not be called a column mismatch: ${E.problems.find((p) => String(p).includes(MISMATCH_SAYS))}`);
...
console.log(`  F  the same header in **bold** ................. ${fFalseAlarm ? 'FLAGGED' : 'clean  '}${fFalseAlarm ? '  <- X193' : ''}`);

const G = verdict(repeatHeader(reqHeader.toUpperCase()));
const gFalseAlarm = flagged(G);
console.log(`  G  the same header in UPPERCASE ................ ${gFalseAlarm ? 'FLAGGED' : 'clean  '}${gFalseAlarm ? '  <- X193' : ''}`);
