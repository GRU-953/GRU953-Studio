...
// Writes a Definition-of-Done record that LOOKS measured, so these tests exercise the row-level
// logic they were written for. quality-gate.mjs now also asks where the table came from (see its
// PROVENANCE arm), and without this every one of them would fail on that one problem instead of
// the row problem under test — which would test the provenance arm seventy times and the row
// logic never. `writeGateUnmeasured` below is the fixture for the provenance arm itself.
function writeGate(dir, table) {
  ...
  const marked =
    `<!-- GENERATED by hooks/dod.mjs from Dev-Memory/evidence/*.json. -->\n` +
    `Generated: ${new Date().toISOString()}\n\n${table}`;
  fs.writeFileSync(path.join(dir, 'Dev-Memory', 'QUALITY-GATE.md'), marked);
  // One evidence file per named row, agreeing with the row. Derived from the table on purpose:
  // a fixture that had to restate every row would drift from the table it accompanies.
  let n = 0;
  for (const line of table.split('\n')) {
    if (!line.trim().startsWith('|')) continue;
    const cells = line.split('|').slice(1, -1).map((c) => c.trim());
    const item = cells[0];
    if (!item || /^:?-+:?$/.test(item) || /^(item|check|dimension|requirement|criterion|gate)$/i.test(item)) continue;
    const status = /^\s*(pass|ok|green|done|met|yes|✅|✓)/i.test(cells[1] || '') ? 'pass' : 'n/a';
    fs.writeFileSync(
      path.join(dir, 'Dev-Memory', 'evidence', `fixture-${n++}.json`),
      JSON.stringify({ dimension: `fixture-${n}`, row: item, kind: 'executed', verdict: status, why: 'fixture', endedAt: stamp }),
    );
  }
}
// The same table with nothing behind it — the state every project was in before dod.mjs existed.
function writeGateUnmeasured(dir, table) {
  fs.mkdirSync(path.join(dir, 'Dev-Memory'), { recursive: true });
...
// can read them, and if something checks BEFORE the run that nothing it will need is missing.
// Otherwise "headless" means "runs until the first question, then stops".
// ---------------------------------------------------------------------------
const GOOD_BRIEF = {
  schemaVersion: 1,
  project: 'Expense Tracker',
  idea: 'A simple app to log what I spend and see a monthly total.',
  tier: { assigned: 'Standard', answers: { remembersUsers: true, handlesSensitiveData: false, integrations: 0 } },
  targetPlatform: 'web (desktop and mobile browser)',
  stack: 'studio-chooses',
  mustHave: ['Add an expense', 'See a monthly total'],
  nonGoals: ['No bank syncing'],
  answeredAt: '2026-08-26',
};
function writeBrief(dir, brief) {
  fs.mkdirSync(path.join(dir, 'Dev-Memory'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'Dev-Memory', 'run-brief.json'), JSON.stringify(brief, null, 2));
}

test('run-brief.mjs: a complete, self-consistent brief clears the gate and renders OBJECTIVE.md', () => {
  const dir = mkTmp('gru-brief-ok-');
  writeBrief(dir, GOOD_BRIEF);
  const r = runScript('run-brief.mjs', dir);
  assert.equal(r.code, 0, r.stdout);
  const rendered = fs.readFileSync(path.join(dir, 'Dev-Memory', 'OBJECTIVE.md'), 'utf8');
  assert.match(rendered, /\*\*Tier:\*\* Standard/, "studio/SKILL.md requires the Tier as one exact on-disk line");
  assert.match(rendered, /GENERATED by hooks\/run-brief\.mjs/);
  fs.rmSync(dir, RM_OPTS);
});

// The check that earns this gate its place. studio/SKILL.md documents the Tier as the output of
// three questions under a stated mapping — made checkable by a 2026-07-10 audit fix precisely
// because "a typical web app" had let almost any request round up to Standard by default. Because
// the rule is written down, the Tier can be re-derived and compared.
test('run-brief.mjs: a Tier that does not follow from its own recorded answers is refused', () => {
  const dir = mkTmp('gru-brief-tier-');
  writeBrief(dir, {
    ...GOOD_BRIEF,
    tier: { assigned: 'Standard', answers: { remembersUsers: true, handlesSensitiveData: true, integrations: 0 } },
  });
  const r = runScript('run-brief.mjs', dir);
  assert.equal(r.code, 1, 'handling money or personal data makes this Complex, not Standard');
  assert.match(JSON.stringify(r.json), /produce Complex/);
  fs.rmSync(dir, RM_OPTS);
});

test('run-brief.mjs: the documented Tier mapping is applied exactly', () => {
  const cases = [
    [{ remembersUsers: false, handlesSensitiveData: false, integrations: 0 }, 'Tiny'],
    [{ remembersUsers: true, handlesSensitiveData: false, integrations: 0 }, 'Standard'],
    [{ remembersUsers: false, handlesSensitiveData: false, integrations: 1 }, 'Standard'],
    [{ remembersUsers: false, handlesSensitiveData: false, integrations: 2 }, 'Complex'],
    [{ remembersUsers: false, handlesSensitiveData: true, integrations: 0 }, 'Complex'],
  ];
  for (const [answers, expected] of cases) {
    const dir = mkTmp('gru-brief-map-');
    writeBrief(dir, { ...GOOD_BRIEF, tier: { assigned: expected, answers } });
    const r = runScript('run-brief.mjs', dir);
    assert.equal(r.code, 0, `${JSON.stringify(answers)} must map to ${expected}: ${r.stdout}`);
    assert.equal(r.json.tierDerivedIndependently, expected);
    fs.rmSync(dir, RM_OPTS);
  }
});

test('run-brief.mjs: nonGoals must be PRESENT even when empty (no silent omission)', () => {
  const missing = mkTmp('gru-brief-nng-');
  const { nonGoals, ...withoutNonGoals } = GOOD_BRIEF;
  writeBrief(missing, withoutNonGoals);
  assert.equal(
    runScript('run-brief.mjs', missing).code,
    1,
    'an absent field cannot be told from a deliberate "nothing is out of scope"',
  );
  fs.rmSync(missing, RM_OPTS);

  const empty = mkTmp('gru-brief-nng2-');
  writeBrief(empty, { ...GOOD_BRIEF, nonGoals: [] });
  assert.equal(runScript('run-brief.mjs', empty).code, 0, 'an explicitly empty list is a decision and is allowed');
  fs.rmSync(empty, RM_OPTS);
});

test('run-brief.mjs: placeholders are refused wherever a real answer is required', () => {
  for (const field of ['idea', 'targetPlatform', 'stack']) {
    for (const value of ['TBD', 'tbd', '', 'n/a', '...']) {
      const dir = mkTmp('gru-brief-ph-');
      writeBrief(dir, { ...GOOD_BRIEF, [field]: value });
      assert.equal(
        runScript('run-brief.mjs', dir).code,
        1,
        `${field} = ${JSON.stringify(value)} is something the run would have to ask about`,
      );
      fs.rmSync(dir, RM_OPTS);
    }
  }
...
test('run-brief.mjs: a missing brief, a missing tier, and an unknown schemaVersion all block', () => {
  const noBrief = mkTmp('gru-brief-none-');
  fs.mkdirSync(path.join(noBrief, 'Dev-Memory'), { recursive: true });
  assert.equal(runScript('run-brief.mjs', noBrief).code, 1);
  fs.rmSync(noBrief, RM_OPTS);

  const noTier = mkTmp('gru-brief-notier-');
  const { tier, ...withoutTier } = GOOD_BRIEF;
  writeBrief(noTier, withoutTier);
  assert.equal(runScript('run-brief.mjs', noTier).code, 1);
  fs.rmSync(noTier, RM_OPTS);

  const badSchema = mkTmp('gru-brief-schema-');
  writeBrief(badSchema, { ...GOOD_BRIEF, schemaVersion: 99 });
  assert.equal(runScript('run-brief.mjs', badSchema).code, 1);
  fs.rmSync(badSchema, RM_OPTS);
});

// The fix loop's cap, enforced as data rather than trusted to an agent's counting ---------------
test('task-ledger.mjs: a task past its attempt cap but still runnable is refused', () => {
  const dir = mkTmp('gru-tl-cap-');
  writeLedger(dir, [{ id: 'T1', title: 'Fix the parser', state: 'todo', attempts: 3 }]);
  const r = runScript('task-ledger.mjs', dir);
  assert.equal(r.code, 1, 'a fix loop past its ceiling has not terminated');
  assert.match(JSON.stringify(r.json), /has spent 3 of 3 permitted attempts/);
  fs.rmSync(dir, RM_OPTS);
});

test('task-ledger.mjs: the same task parked as blocked-on-defect lets the run move on', () => {
  const dir = mkTmp('gru-tl-capok-');
  writeLedger(dir, [
    { id: 'T1', title: 'Fix the parser', state: 'blocked-on-defect', attempts: 3, blockedReason: 'tokeniser needs redesigning' },
    { id: 'T2', title: 'Unrelated work', state: 'todo' },
  ]);
  const r = runScript('task-ledger.mjs', dir);
  assert.equal(r.code, 0);
  assert.equal(r.json.next, 'T2', 'the run spends its remaining budget on work it can finish');
  assert.deepEqual(r.json.atAttemptCap, ['T1']);
  fs.rmSync(dir, RM_OPTS);
});

test('task-ledger.mjs: a project may declare its own cap, and a nonsense one blocks', () => {
  const dir = mkTmp('gru-tl-capcfg-');
  writeLedger(dir, [{ id: 'T1', title: 'x', state: 'in-progress', attempts: 1 }]);
  fs.writeFileSync(
    path.join(dir, 'Dev-Memory', 'run.json'),
    JSON.stringify({ schemaVersion: 1, maxAttemptsPerTask: 1 }),
  );
  assert.equal(runScript('task-ledger.mjs', dir).code, 1, 'a declared cap of 1 must be honoured');
  fs.writeFileSync(
    path.join(dir, 'Dev-Memory', 'run.json'),
    JSON.stringify({ schemaVersion: 1, maxAttemptsPerTask: 0 }),
  );
  assert.equal(runScript('task-ledger.mjs', dir).code, 1, 'a cap of 0 is not a cap');
  fs.rmSync(dir, RM_OPTS);
});
...
// ---- the shell surface (2026-08-27 contract sweep) --------------------------------------
// This hook was wired to Write|Edit|MultiEdit|NotebookEdit only, so every file above was one
// redirection away from being written anyway: the same evidence file refused as an Edit was
// allowed as `echo '{"verdict":"pass"}' > Dev-Memory/evidence/tests.json`. A guard on four of
// the five ways to write a file is not a guard, and the tests above proved only the four.
function cfgProject(prefix) {
  const dir = mkTmp(prefix);
  fs.mkdirSync(path.join(dir, 'Dev-Memory', 'evidence'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'Dev-Memory', 'dod.json'), '{}\n');
  fs.writeFileSync(path.join(dir, 'Dev-Memory', 'QUALITY-GATE.md'), '# q\n');
  fs.writeFileSync(path.join(dir, 'Dev-Memory', 'evidence', 'tests.json'), '{"verdict":"pass"}\n');
  return dir;
}

for (const [label, command] of [
  ['a redirection', 'echo x > Dev-Memory/evidence/tests.json'],
  ['an append', 'echo x >> Dev-Memory/evidence/tests.json'],
  ['a redirection behind a harmless first command', 'npm test && echo x > Dev-Memory/dod.json'],
  ['tee', 'cat new | tee Dev-Memory/QUALITY-GATE.md'],
  ['an in-place sed', 'sed -i "" s/80/0/ Dev-Memory/dod.json'],
  ['an in-place perl', 'perl -i -pe s/80/0/ Dev-Memory/dod.json'],
  ['a copy onto it', 'cp /tmp/x Dev-Memory/evidence/tests.json'],
  ['a move onto it', 'mv /tmp/x Dev-Memory/evidence/tests.json'],
  ['a deletion', 'rm Dev-Memory/evidence/tests.json'],
  // Found while reviewing the fix above: the shell arm guarded writes to FILES, and the two
  // commonest destructive spellings name a DIRECTORY or move it away. Every individual evidence
  // file was protected while `rm -rf Dev-Memory/evidence` removed all of them at once.
  ['deleting the whole evidence directory', 'rm -rf Dev-Memory/evidence'],
  ['deleting it with a trailing slash', 'rm -rf Dev-Memory/evidence/'],
  ['moving the evidence away', 'mv Dev-Memory/evidence /tmp/gone'],
  ['renaming the Definition of Done out of the way', 'mv Dev-Memory/dod.json Dev-Memory/dod.json.bak'],
  ['a quoted path', 'echo x > "Dev-Memory/dod.json"'],
]) {
  test(`config-protection.mjs: DENIES ${label} into the measurement substrate`, () => {
    const dir = cfgProject('gru-cfg-sh-');
    const r = runHook('config-protection.mjs', command, dir);
    assert.equal(r.decision, 'deny', `${label} must be refused: ${command}`);
    fs.rmSync(dir, RM_OPTS);
  });
}

// The controls. A guard that fires on ordinary work is a guard that gets switched off, and these
// are the commands a build runs constantly.
for (const [label, command] of [
  ['an ordinary build command', 'npm test && git status'],
  ['writing an ordinary project file', 'echo hi > src/index.js'],
  ['READING the Definition of Done', 'cat Dev-Memory/dod.json'],
  ['grepping the evidence', 'grep -r verdict Dev-Memory/evidence/'],
  // COPYING the substrate out is reading it, and taking a backup before a risky change is
  // ordinary good practice. Only `mv` claims every operand, because only `mv` removes its source.
  ['copying the Definition of Done out as a backup', 'cp Dev-Memory/dod.json /tmp/backup.json'],
  ['renaming ordinary source files', 'mv src/a.js src/b.js'],
  ['clearing build output', 'rm -rf node_modules dist'],
  ['running the gate itself', 'node hooks/dod.mjs .'],
]) {
  test(`config-protection.mjs: stands aside for ${label}`, () => {
    const dir = cfgProject('gru-cfg-sh-ok-');
    const r = runHook('config-protection.mjs', command, dir);
    assert.equal(r.decision, null, `${label} must not be interfered with: ${command}`);
    fs.rmSync(dir, RM_OPTS);
  });
}

// A relative path in a tool payload is relative to the SESSION's directory, not to wherever the
// hook runs. Resolving against the wrong root made statSync fail, which this hook reads as
// first-time creation and waves through — so the guard was failing open on its commonest input.
test('config-protection.mjs: resolves a relative path against the tool call’s cwd, not its own', () => {
  const dir = cfgProject('gru-cfg-cwd-');
  const input = JSON.stringify({
    tool_name: 'Edit',
    tool_input: { file_path: 'Dev-Memory/evidence/tests.json' },
    cwd: dir,
  });
...
// ---------------------------------------------------------------------------
// 2026-08-27 contract sweep. Eighteen findings survived adversarial verification; these are the
// tests for the ones that were fixed here. The common shape of all of them: a gate that reads
// what the graded work SAYS rather than what it DID, or a mechanism the product never invokes.
// ---------------------------------------------------------------------------

// THE ONE THAT MATTERS MOST. The Definition-of-Done table copied verbatim out of the
// quality-gate skill's own worked example, in a Dev-Memory holding nothing else at all,
// returned {"status":"clean"} and exit 0 — and publish-github's pre-flight ran exactly this
// gate to clear a release. The agents write that table.
test('quality-gate.mjs: an ATTESTED table with no measurements behind it is refused (2026-08-27)', () => {
  const dir = mkTmp('gru-qg-attested-');
  writeGateUnmeasured(dir, FULL_DOD);
  const r = runScript('quality-gate.mjs', dir);
  assert.notEqual(r.code, 0, `a table nobody measured must not clear the gate, got: ${r.stdout}`);
  assert.match(
    JSON.stringify(r.json),
    /no measurements behind it/,
    'the reason must name what is missing, not merely refuse',
  );
  fs.rmSync(dir, RM_OPTS);
});

// The inverse control, and the one that matters second-most: the fix must not block a project
// that DID the work. A gate that cannot be satisfied gets switched off.
test('quality-gate.mjs: a genuinely measured project still passes (inverse control)', () => {
  const dir = mkTmp('gru-qg-measured-');
  writeGate(dir, FULL_DOD);
  const r = runScript('quality-gate.mjs', dir);
  assert.equal(r.code, 0, `a measured project must still be clean, got: ${r.stdout}`);
  fs.rmSync(dir, RM_OPTS);
});

test('quality-gate.mjs: a row added by hand beneath a generated table has no evidence and is named', () => {
  const dir = mkTmp('gru-qg-handrow-');
  writeGate(dir, FULL_DOD);
  const p = path.join(dir, 'Dev-Memory', 'QUALITY-GATE.md');
  fs.appendFileSync(p, '\n| Penetration testing | pass | pentest report attached |\n');
  const r = runScript('quality-gate.mjs', dir);
  assert.notEqual(r.code, 0);
  assert.match(JSON.stringify(r.json), /Penetration testing.*no measurement behind it/);
  fs.rmSync(dir, RM_OPTS);
});

test('quality-gate.mjs: a measurement that FAILED contradicts a table reporting the bar as met', () => {
  const dir = mkTmp('gru-qg-contradict-');
  writeGate(dir, FULL_DOD);
  const evidence = path.join(dir, 'Dev-Memory', 'evidence');
  const f = fs.readdirSync(evidence)[0];
  const rec = JSON.parse(fs.readFileSync(path.join(evidence, f), 'utf8'));
  fs.writeFileSync(path.join(evidence, f), JSON.stringify({ ...rec, verdict: 'fail', why: 'exit 1' }));
  const r = runScript('quality-gate.mjs', dir);
  assert.notEqual(r.code, 0);
  assert.match(JSON.stringify(r.json), /records .?fail.?.*table reports this Definition of Done as met/);
  fs.rmSync(dir, RM_OPTS);
});

test('quality-gate.mjs: a table older than its own evidence is STALE and refused', () => {
  const dir = mkTmp('gru-qg-stale-');
  writeGate(dir, FULL_DOD);
  const evidence = path.join(dir, 'Dev-Memory', 'evidence');
  const f = fs.readdirSync(evidence)[0];
  const rec = JSON.parse(fs.readFileSync(path.join(evidence, f), 'utf8'));
  // A measurement taken AFTER the table was rendered: the table cannot be showing it.
  fs.writeFileSync(
    path.join(evidence, f),
    JSON.stringify({ ...rec, endedAt: new Date(Date.now() + 3_600_000).toISOString() }),
  );
  const r = runScript('quality-gate.mjs', dir);
  assert.notEqual(r.code, 0);
  assert.match(JSON.stringify(r.json), /STALE/);
  fs.rmSync(dir, RM_OPTS);
});
...
test('dod.mjs: a coverage floor of 0 is refused — a check that cannot fail is not a check', () => {
  ...
  assert.match(JSON.stringify(r.json), /cannot fail and therefore is not a check/);
  fs.rmSync(dir, RM_OPTS);
});
...
test('dod.mjs: a floor below the product minimum needs a stated reason, and then is allowed', () => {
  ...
  const r1 = runScript('dod.mjs', bare);
  assert.notEqual(r1.code, 0);
  assert.match(JSON.stringify(r1.json), /below this product's minimum floor of 60/);
  fs.rmSync(bare, RM_OPTS);

  const declared = mk({
    minPercent: 30,
    reportPath: 'c.json',
    floorBelowProductMinimumBecause: 'generated protobuf bindings dominate this package',
  });
  const r2 = runScript('dod.mjs', declared);
  assert.equal(r2.code, 0, `a recorded exception must be honoured, got: ${r2.stdout}`);
  fs.rmSync(declared, RM_OPTS);
});

test('dod.mjs: a command that cannot measure anything is refused (["true"] passed all ten)', () => {
  const dir = mkTmp('gru-dod-vacuous-');
  writeDod(dir, { ...DOD_ALL_NA, tests: { command: ['true'] } });
  const r = runScript('dod.mjs', dir);
  assert.notEqual(r.code, 0);
  assert.match(JSON.stringify(r.json), /cannot measure tests/);
  fs.rmSync(dir, RM_OPTS);
});

test('dod.mjs: a cwd outside the project is refused — the parent repo’s suite is not this one’s', () => {
  const dir = mkTmp('gru-dod-cwd-');
  writeDod(dir, { ...DOD_ALL_NA, tests: { command: [process.execPath, '-e', ''], cwd: '../..' } });
  const r = runScript('dod.mjs', dir);
  assert.notEqual(r.code, 0);
  assert.match(JSON.stringify(r.json), /outside the project root/);
  fs.rmSync(dir, RM_OPTS);
});

test('dod.mjs: a coverage reportPath outside the project is refused', () => {
  const dir = mkTmp('gru-dod-rp-');
  writeDod(dir, {
    ...DOD_ALL_NA,
  ...
  const r = runScript('dod.mjs', dir);
  assert.notEqual(r.code, 0);
  assert.match(JSON.stringify(r.json), /reportPath.*outside the project root/);
  fs.rmSync(dir, RM_OPTS);
});

// `notApplicable` accepted any non-empty string, so the whole Definition of Done could be waived
// one sentence at a time — twelve reasons, nothing run, exit 0 — and no reason had to be true.
test('dod.mjs: an n/a the project itself contradicts is refused, and an honest one is not', () => {
  const withTests = mkTmp('gru-dod-na-');
  fs.writeFileSync(
    path.join(withTests, 'package.json'),
  ...
  // tree, never from the prose — otherwise this becomes a gate that judges sentences.
  const without = mkTmp('gru-dod-na-ok-');
...
// so anyone writing dod.json was guessing at a format a gate would reject — which unattended is a
// run that stops on a question no one is there to answer. Now documented; pinned here so the
// documentation and the enforcement cannot drift apart.
test('the dod.json contract documented in skills/quality-gate/SKILL.md matches what dod.mjs enforces', () => {
  const skill = fs.readFileSync(path.join(HERE, '..', 'skills', 'quality-gate', 'SKILL.md'), 'utf8');
  const block = /```json\n([\s\S]*?)```/.exec(skill);
  assert.ok(block, 'the skill must carry a worked dod.json example — a contract with no example is a guess');
  const doc = JSON.parse(block[1]);

  const src = fs.readFileSync(path.join(HERE, 'dod.mjs'), 'utf8');
  const version = /const SCHEMA_VERSION = (\d+)/.exec(src);
  assert.ok(version, 'dod.mjs must declare SCHEMA_VERSION');
  assert.equal(
    doc.schemaVersion,
    Number(version[1]),
    'the documented schemaVersion must be the one the gate accepts',
  );

  const keysIn = (constName) => {
    const m = new RegExp(`const ${constName} = \\[([\\s\\S]*?)\\n\\];`).exec(src);
    assert.ok(m, `dod.mjs must declare ${constName}`);
    return [...m[1].matchAll(/key: '([a-z]+)'/g)].map((x) => x[1]);
  };
  const enforced = [...keysIn('EXECUTED'), ...keysIn('JUDGED')].sort();
  assert.deepEqual(
    Object.keys(doc.dimensions).sort(),
    enforced,
    'the documented example must name every dimension the gate requires, and no others',
  );

  assert.ok(typeof doc.dimensions.coverage.minPercent === 'number');
  assert.ok(typeof doc.dimensions.coverage.reportPath === 'string');
  assert.equal(doc.dimensions.review.verdict, 'pass');
  assert.ok(typeof doc.dimensions.review.artefact === 'string' && doc.dimensions.review.artefact !== '');
  for (const [key, spec] of Object.entries(doc.dimensions)) {
    if (spec.command !== undefined) {
      assert.ok(
        Array.isArray(spec.command) && spec.command.every((c) => typeof c === 'string'),
        `${key}: the documented command must be an argv array — a string would have to reach a shell`,
      );
    }
  }
});
...
// DC11, 2026-08-27 (contract sweep). `/studio-resume` told the studio to "set its Status back to
// `doing`" — a state hooks/task-ledger.mjs REFUSES — so obeying the resume command produced a
// ledger the next gate blocked on, and the whole pause/resume cycle dead-ended at the step whose
// only job was to undo the pause. Four other command files and three skills carried the same
// retired vocabulary, including one skill that announced the rename on line 26 and then used the
// old word further down. Nothing could have caught it: the accepted states lived in a Set inside
// a hook, the instructed states lived in prose, and no gate compared the two.
test('docs-consistency.mjs: DC11 refuses a command that instructs a task state the ledger rejects', () => {
  const dir = mkTmp('gru-dc11-');
  copyRepoTo(dir);
  const f = path.join(dir, 'plugins', 'gru953-studio', 'commands', 'studio-pause.md');
  const before = fs.readFileSync(f, 'utf8');
  assert.match(before, /`in-progress` row/, 'precondition: the file must currently use the live state name');
  fs.writeFileSync(f, before.replace('`in-progress` row', '`doing` row'));
  const r = runDocsConsistency(dir);
  assert.notEqual(r.status, 0, 'a state the ledger refuses must not pass unremarked');
  assert.ok(
...
test('docs-consistency.mjs: DC11 permits a retired state named on a line that says it is retired', () => {
  const dir = mkTmp('gru-dc11-ok-');
  copyRepoTo(dir);
  const f = path.join(dir, 'plugins', 'gru953-studio', 'commands', 'studio-pause.md');
  fs.appendFileSync(f, '\n(Historical note: `doing` is now `in-progress`.)\n');
  const r = runDocsConsistency(dir);
  assert.equal(r.json && r.json.status, 'clean', `explaining the rename must not be blocked, got: ${r.stdout}`);
  fs.rmSync(dir, RM_OPTS);
});
...
test('repo-integrity.mjs: INV12 requires the Publish protocol to name dod.mjs and task-ledger.mjs', () => {
  for (const hook of ['dod.mjs', 'task-ledger.mjs']) {
    ...
    copyRepoTo(dir);
    const f = path.join(dir, 'plugins', 'gru953-studio', 'skills', 'publish-github', 'SKILL.md');
    const before = fs.readFileSync(f, 'utf8');
    assert.ok(before.includes(`hooks/${hook}`), `precondition: the protocol must currently name ${hook}`);
    fs.writeFileSync(f, before.split(`hooks/${hook}`).join('hooks/licence-scan.mjs'));
    const r = runScript('repo-integrity.mjs', dir);
    assert.notEqual(r.code, 0, `dropping ${hook} from the Publish protocol must fail INV12`);
    assert.ok(
      JSON.stringify(r.json).includes(hook),
      `the failure must name ${hook}, got: ${r.stdout.slice(0, 400)}`,
    );
    fs.rmSync(dir, RM_OPTS);
  }
});
