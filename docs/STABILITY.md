...
| **That no hook contains a network client**                         | No hook calls `fetch` (member access and aliases included), imports `http`, `https`, `http2`, `net`, `dns`, `tls` or `dgram`, constructs `WebSocket`, `XMLHttpRequest` or `EventSource`, or loads a module by a computed specifier. `docs-consistency.mjs` DC14 checks it on every commit, over the whole `hooks/` tree, on source with comments and strings masked out. What it catches is the accident — a client coming back the way v6's `openrouter-models.mjs` did, unnoticed; it is not proof against a determined author, and that limit is stated rather than implied. It is also deliberately narrower than "the plugin makes no network call", which this row used to say and which was not true — see the note under the table. |
